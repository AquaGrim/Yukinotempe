...
async function afkHandler(client, msg) {
  try {
    ...
    if (!chat || !chat.id || !chat.id._serialized) {
      console.warn(`⚠️ Gagal mengambil chat dari ${msg.from}, mungkin pesan pribadi atau data belum siap.`);
      return false;
    }
...
const groupId = chat.id._serialized;
  ...
  try {
    ...
    if (quotedMsg) {
      const repliedUserId = quotedMsg.author || quotedMsg.from;
      mentionedUsers.push(repliedUserId);
    }
  } catch (e) {
    console.warn("⚠️ Gagal mengambil quoted message:", e.message);
  }
}

    for (const targetId of mentionedUsers) {
      const targetKey = isGroup ? `${targetId}@${chat.id._serialized}` : targetId;
