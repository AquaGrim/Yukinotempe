...
const shopItems = {
  ...
  buff: {
    name: "Buff Scroll",
    price: 75,
    desc: "+25% peluang menang di !lawan",
  },
...
async function buyHandler(msg) {
  try {
    ...
    if (quantity < 1 || isNaN(quantity))
      return msg.reply("   ^}^r Jumlah item tidak valid.");

    const totalPrice = item.price * quantity;
    if (user.money < totalPrice) {
      return msg.reply(
        `   ^}^r Uangmu tidak cukup. Harga total: ${totalPrice} gold.`
      );
    }
    ...
    saveUsers(users);
    await msg.reply(
      `   ^|^e Kamu membeli *${quantity} ${item.name}* seharga *${totalPrice} gold*.`
    );
  } catch (error) {
    console.error("[SHOP] Error:", error.message);
    return msg.reply("❌ Terjadi kesalahan saat membeli item.");
  }
}
