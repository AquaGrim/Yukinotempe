...
async function useItemHandler(msg) {
  ...
  if (args.length === 0) return msg.reply('Gunakan:\n- !pakai [item] [jumlah/all]\n- !pakai all');
  ...
  if (firstArg === 'all') {
    ...
    if (!anyUsed) return msg.reply('   ^}^l Kamu tidak punya item yang bisa digunakan.');
    saveUsers(users);
    return;
  }
  ...
  if (!itemName) return msg.reply('   ^}^s Item tidak dikenali atau belum didukung.');
...
async function applyItemEffect(user, itemName, quantity, msg) {
  switch (itemName) {
    case 'Potion':
      ...
      return msg.reply(`   ^=^n^e ${user.name} menggunakan ${quantity} Potion. +${staminaGain} Stamina`);
    case 'Gold Coin':
      const goldGain = 125 * quantity;
      user.money += goldGain;
      return msg.reply(`   ^=^r ${user.name} menggunakan ${quantity} Gold Coin. +${goldGain} Gold`);
    case 'Buff Scroll':
      user.buff = true;
      return msg.reply(`   ^|^e ${user.name} menggunakan ${quantity} Buff Scroll. Peluang menang meningkat untuk pertarungan berikutnya.`);
    case 'Exp Book':
