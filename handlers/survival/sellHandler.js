...
- !jual [nama item] [jumlah]    ^f^r contoh: !jual lele 2`);
    ...
    const itemData = sellables.find(
      (i) => i.name.toLowerCase() === nameRaw.toLowerCase()
    );

    if (!itemData) {
      return msg.reply(
        `   ^}^l Item '${nameRaw}' tidak ditemukan dalam daftar jual.`
      );
    }
    ...
    return msg.reply(
      `   ^o Kamu menjual ${amount} ${itemData.name} dan mendapat ${earned} gold.`
    );
  } catch (error) {
    console.error("[SELL] Error:", error.message);
    return msg.reply("❌ Terjadi kesalahan saat menjual item.");
  }
}
