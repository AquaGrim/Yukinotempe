...
if (rawText.startsWith("!brat")) {
  ...
  while (lines.length * fontSize * 1.2 > (canvasHeight - 2 * padding) && fontSize > 10) {
    fontSize -= 2;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const spaceWidth = spaceCount > 0 ? (maxWidth - totalWordsWidth) / spaceCount : 0;

    let x = padding;
    for (let j = 0; j < wordsInLine.length; j++) {
      svgLines += `<tspan x="${x.toFixed(1)}" y="${(yStart + i * lineHeight).toFixed(1)}">${wordsInLine[j]}</tspan>\n`;
  ...
  return true;
    ...
    if (lowerText === '!sticker') {
      ...
      if (!targetMsg.hasMedia) {
        ...
        await msg.reply('❌ Harap reply atau kirim gambar dengan caption `!sticker`.');
        return true;
      }
      const media = await targetMsg.downloadMedia();
      if (!media) {
        console.error('[STICKER] Failed to download media from target message.');
        await msg.reply('⚠️ Gagal mengambil media.');
        return true;
      }
      if (media.mimetype) {
        if (!media.mimetype.includes('image')) {
          console.error('[STICKER] Downloaded media is not an image, mimetype:', media.mimetype);
          await msg.reply('❌ Harap reply atau kirim gambar dengan caption `!sticker`.');
          return true;
        }
      } else {
        console.warn('[STICKER] media.mimetype not available; assuming media is an image.');
      }
      console.log('[STICKER] Media successfully downloaded, sending as sticker.');
    ...
    const isConvertTrigger = convertTriggers.some(trigger =>
      lowerText === trigger || lowerText.startsWith(trigger) || lowerText.includes(trigger)
    );
    if (isConvertTrigger) {
      let stickerMsg;
      if (msg.hasQuotedMsg) {
      ...
      if (!stickerMsg) {
        ...
        await msg.reply("❌ Harap reply ke stiker yang ingin dikonversi atau kirim stiker dengan trigger konversi.");
        return true;
      }
      ...
      const ext = (stickerMsg.mimetype && stickerMsg.mimetype.includes('webp')) ? '.webp' : '.png';
      ...
      console.log('[CONVERT] Sticker data written to temporary file:', tempInput);
      try {
        await sharp(tempInput)
          .flatten({ background: { r: 255, g: 255, b: 255 } })
          .jpeg({ quality: 90 })
          .toFile(tempOutput);
        console.log('[CONVERT] File successfully converted to JPEG:', tempOutput);
        const outputBase64 = fs.readFileSync(tempOutput).toString('base64');
        const outputMedia = new MessageMedia('image/jpeg', outputBase64, 'converted.jpg');
        await msg.reply(outputMedia, null, { caption: '🖼️ Stiker berhasil dikonversi ke gambar.' });
